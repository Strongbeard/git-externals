...
class GitExternalsTestCase(BaseGitExternalsTestCase):
    ...
    def test_single_svn_external(self):
        ...
        git_external = git_externals.GitExternal(git_dir)
        git_external.add_external(
            f"file://{svn_remote_proj_trunk_dir!s}", svn_local_dir.name, vcs="svn"
        )

        # Assert GitExternals properly linked to external svn repo
        externals_file_path = git_dir.joinpath(".gitexternals")
        self.assertTrue(
            externals_file_path.exists(), f".gitexternals is missing from {git_dir!s}"
        )
        ...
        self.assertTrue(
            gitignore_file_path.exists(), f".gitignore is missing from {git_dir!s}"
        )
    ...
    def test_recursive_svn_externals(self):
        ...
        self.assertTrue(
            git_dir.joinpath(
                f"{svn_local_outer_proj_dir.name}/{svn_local_outer_proj_dir.name}.txt"
            ).exists()
        )
        self.assertTrue(
            git_dir.joinpath(
                f"{svn_local_outer_proj_dir.name}/{svn_local_inner_proj_dir.name}/{svn_local_inner_proj_dir.name}.txt"
            ).exists()
        )
        self.assertTrue(
            git_dir.joinpath(f"{svn_local_outer_proj_dir.name}/.svn").exists()
        )
