...
def _cmd_show(git_external: GitExternal, **kwargs):
    ...
    for repo, config in git_external.configurations.items():
        ...
        if (
            kwargs["recursive"]
            and pathlib.Path(config["path"]).joinpath(".gitexternals").exists()
        ):
...
def _main(*args):
    ...
    parser = argparse.ArgumentParser(
        prog=args[0], description=sys.modules[__name__].__doc__
    )
    ...
    parser_update.add_argument(
        "external", nargs="?", default=None, help="Name of external to update"
    )

    # subcommand CLONE
    parser_clone = subparsers.add_parser("clone")
    parser_clone.set_defaults(
        func=_cmd_update, git_external=git_external, only=("clone",)
    )
    ...
    parser_add.add_argument(
        "-b", "--branch", default="master", help="Branch that should be used"
    )
    parser_add.add_argument(
        "--script", default=None, help="Script to run after cloning the external"
    )
